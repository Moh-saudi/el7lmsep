...
export default function TestBeOnCorrectMethodPage() {
  const [result, setResult] = useState<any>(null);
  const [loading, setLoading] = useState(false);

  const testCorrectMethod = async () => {
    setLoading(true);
    setResult(null);
    
    try {
      const baseUrl = 'https://v3.api.beon.chat';
      const token = 'yK1zYZRgjvuVC5wJcmkMwL0zFsRi9BhytEYPXgnzbNCyPFkaJBp9ngjmO6q4';
      const endpoint = '/api/v3/messages/otp';
      
      console.log('🧪 اختبار الطريقة الصحيحة:', `${baseUrl}${endpoint}`);
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">
          ✅ اختبار BeOn API بالطريقة الصحيحة
        </h1>

        <div className="bg-white rounded-lg shadow p-6 mb-8">
          <h2 className="text-xl font-semibold mb-4">🎯 الطريقة الصحيحة:</h2>
          <div className="bg-gray-100 p-4 rounded font-mono text-sm space-y-2">
            ...
            <div><strong>Content-Type:</strong> multipart/form-data (FormData)</div>
            <div><strong>Headers:</strong> beon-token فقط</div>
            <div><strong>Body:</strong> FormData مع phoneNumber, name, type, otp_length, lang</div>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow p-6 mb-8">
          <h2 className="text-xl font-semibold mb-4">📋 مثال cURL:</h2>
          <pre className="bg-gray-100 p-4 rounded text-sm overflow-auto">
{`curl -X POST "https://v3.api.beon.chat/api/v3/messages/otp" \\
  -H "Content-Type: multipart/form-data" \\
  -H "beon-token: yK1zYZRgjvuVC5wJcmkMwL0zFsRi9BhytEYPXgnzbNCyPFkaJBp9ngjmO6q4" \\
