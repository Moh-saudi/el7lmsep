...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log('📱 === بدء طلب SMS Regular ===');
    console.log('🕐 وقت البداية:', new Date().toISOString());
    
    const { phoneNumber, message, name = 'El7lm' } = await request.json();

    console.log('📱 البيانات المستلمة:', { phoneNumber, messageLength: message?.length, name });
    ...
    if (process.env.NODE_ENV === 'development' && process.env.ENABLE_SMS_SIMULATION === 'true') {
      console.log('🔧 وضع التطوير - محاكاة SMS Regular');
      ...
      console.log('✅ محاكاة SMS Regular ناجحة');
      console.log('⏱️ وقت الاستجابة:', responseTime + 'ms');
      console.log('📱 === انتهاء طلب SMS Regular ===');
    ...
    const SMS_ENDPOINT = 'https://v3.api.beon.chat/api/v3/messages/sms';

    console.log('🔧 تكوين SMS Regular:', {
      token: BEON_TOKEN.substring(0, 10) + '...',
      endpoint: SMS_ENDPOINT,
      name: name
    });

    console.log('📱 إرسال SMS Regular إلى:', formattedPhoneNumber);
    console.log('📱 الرسالة:', message);
    
    // إرسال عبر SMS Regular باستخدام API الصحيح
    ...
    console.log('📱 استجابة API SMS Regular:', {
    ...
    try {
      ...
      if (responseBodyText) {
        console.log('📄 SMS Regular Provider body:', responseBodyText);
      } else {
        console.log('📄 SMS Regular Provider body: <empty>');
      }
    } catch (e) {
    ...
    if (response.ok && !isProviderError) {
      console.log('✅ SMS Regular تم إرساله بنجاح إلى:', formattedPhoneNumber);
      console.log('📱 === انتهاء طلب SMS Regular ===');
      ...
      console.error('❌ فشل إرسال SMS Regular (مزود):', errorPayload);
      console.log('📱 === انتهاء طلب SMS Regular ===');
    ...
    const responseTime = Date.now() - startTime;
    console.error('❌ خطأ في SMS Regular:', error);
    console.log('📱 === انتهاء طلب SMS Regular ===');
